...
def index(request):
    ...
    return render(request, 'ads/index.html', {'queryset': queryset,
                                              'filter': ad_filter,
                                              'ads': ads})
...
@login_required
def contact(request, slug, recipient=None, success_url=None,
    ...
    if request.method == "POST":
        ...
        form = ComposeForm(initial={'recipient': row.user.username,
                                    'subject': row.title})

        if recipient is not None:
            recipients = [u for u in User.objects.filter(
                **{'%s__in' % get_username_field(): [r.strip()
                                                     for r in
                                                     recipient.split('+')]})]
...
class AttachmentDeleteView(DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        self.object.delete()
        success_url = reverse('ads:edit', kwargs={'pk': self.ad.id})
        return redirect(success_url)
